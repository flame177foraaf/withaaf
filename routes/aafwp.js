...
router.get('/:id', async function(req,res,next) {
  ...
  if (searchtype != '1stats' && searchtype != '2stats') {
    ...
    if (Search2 !== undefined) {
      ...
      var QueryString = "SELECT DISTINCT wpname,*, count(*) over() as totalcount  FROM (select *from aquafeq.aquafwp where "+ searchtype +" Ilike $1 ) AS t1 LEFT JOIN (select name, effect From aquafeq.realize_atk) AS t2 ON t1.wpname = t2.name WHERE t1."+ searchtype +" Ilike $1 " + SearchPlus + " ORDER BY wplimit,wpid asc limit 10 offset (($2- 1)*10);";
      ...
      var QueryString = "SELECT DISTINCT wpname, *, count(*) over() as totalcount  FROM (select *from aquafeq.aquafwp where "+ searchtype +" Ilike $1 ) AS t1 LEFT JOIN (select name, effect From aquafeq.realize_atk) AS t2 ON t1.wpname = t2.name WHERE t1."+ searchtype +" Ilike $1 ORDER BY wplimit,wpid asc limit 10 offset (($2- 1)*10);";
