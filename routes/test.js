...
router.get('/:id', async function(req,res,next) {
  ...
  if (searchtype != '1stats' && searchtype != '2stats') {
    ...
    if (Search2 !== undefined) {
      ...
      var QueryString = "SELECT * FROM aquafeq.aquafwp AS t1 LEFT JOIN  (SELECT effect FROM aquafeq.realize_atk) AS t2 ON t1.wpname = t2.name WHERE " + searchtype +" Ilike $1 " + SearchPlus + " ORDER BY wplimit,wpid asc limit 10 offset (($2- 1)*10);"
